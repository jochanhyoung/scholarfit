...
export function FormPage() {
  ...
  const nextStep = () => {
    if (currentStep < TOTAL_STEPS) {
      ...
      // Persist profile to Supabase if a session exists
      supabase.auth.getSession().then(async ({ data: { session } }) => {
        if (session) {
          const { error } = await supabase.from("users").upsert(
            {
              auth_user_id: session.user.id,
              is_freshman: formData.isFirstYear,
              grade: formData.year,
              gpa_scale: parseFloat(formData.gradeScale),
              last_gpa: formData.lastSemesterGrade ? parseFloat(formData.lastSemesterGrade) : null,
              total_gpa: formData.totalGrade ? parseFloat(formData.totalGrade) : null,
              income_decile: formData.incomeBracket,
              gender: formData.gender === "none" ? null : formData.gender,
            },
            { onConflict: "auth_user_id" }
          );
          if (import.meta.env.DEV) console.log("[DEV] profile upsert error:", error);
        }
      });
      // Navigate to results
