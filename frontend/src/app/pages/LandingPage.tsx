...
import { Coins, Sparkles, ArrowRight, Trash2, User, LogOut } from "lucide-react";
    ...
    const loadSavedData = () => {
      try {
        ...
        if (saved) {
          ...
          const hasGrade = data.isFirstYear || (data.lastSemesterGrade && data.totalGrade);
        ...
        {user ? (
          <>
            <button
              ...
              className="bg-white border-2 border-[#3282F6] text-[#3282F6] h-[48px] px-5 rounded-[16px] hover:bg-[#E8F0FE] transition-all duration-300 active:scale-95 flex items-center gap-2 shadow-lg"
            ...
            >
              ...
              <span className="hidden sm:inline max-w-[100px] truncate">{user.name ?? "내 정보"}</span>
            </button>
            <button
              onClick={signOut}
              className="bg-white border-2 border-gray-200 text-gray-500 h-[48px] px-3 rounded-[16px] hover:bg-gray-50 transition-all duration-300 active:scale-95 flex items-center shadow-lg"
              title="로그아웃"
            >
