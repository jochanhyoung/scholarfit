...
const TEST_GRADES: Record<string, string[]> = {
...
export function Step5Language({ data, updateData }: StepProps) {
  ...
  const updateLanguageScore = (index: number, field: 'test' | 'score', value: string) => {
    ...
    if (field === 'test') {
      newScores[index] = { ...newScores[index], test: value, score: "" };
    } else if (field === 'score') {
      const testType = newScores[index].test;
      const isGradeBased = testType in TEST_GRADES;
      
      // Only restrict to numbers/decimals if it's NOT a grade-based test
      if (!isGradeBased) {
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block mb-2 text-sm text-gray-600">
                    {score.test in TEST_GRADES ? "등급" : "점수"}
                  </label>
                  {score.test in TEST_GRADES ? (
                    <select
                      value={score.score}
                    ...
                    >
                      <option value="">선택해주세요</option>
                      {TEST_GRADES[score.test].map((grade) => (
